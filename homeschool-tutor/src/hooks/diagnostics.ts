 ...
 *
 * The outage below happened in the sibling `demo/` app, not here. It is
 * mirrored into this app deliberately: the gap it exposed is identical on
 * both sides (see this file's twin at demo/src/diagnostics.ts), and this
 * app is the one that runs on a child's tablet on a home LAN, where nobody
 * has devtools, the parent is not going to read a console, and a silent
 * network failure is indistinguishable from "Bede is being slow". If
 * anything the case for always-on diagnostics is stronger here.
 *
 * On 2026-08-04 the public demo could not start a session. The only thing a
