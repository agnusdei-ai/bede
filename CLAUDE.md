...
routers/
  ...
  feedback.py        GET /feedback/enabled (public, gates whether the frontend shows any feedback UI at all); POST /feedback — any authenticated role (parent, child, demo visitor), routed via `services/email_service.py` to `FEEDBACK_EMAIL` over Resend, never persisted server-side beyond that one outbound email. `FeedbackRequest.category` (`models/schemas.py`) covers ordinary in-use feedback (cx/ux/content_quality/other), the demo's own "interested in plans" lead capture (`plans`) and end-of-session survey (`beta_close`), and a real beta family's one-time setup-completion intake (`onboarding` — see `ParentSetup.tsx`/`BetaIntakeModal.tsx` above) — each gets its own email subject-line prefix (`_feedback_prefix`) so the operator's inbox stays triageable at a glance. POST /feedback/lead is a second, deliberately separate entry point into that same `plans`-category pipeline: unauthenticated (`LeadRequest`, `models/schemas.py` — email + optional message + a honeypot field), for the marketing site's (`site/index.html`) "notify me" form, which has no session to gate behind `require_auth` at all. Leans on a honeypot (silent no-op, fake success, if filled) and its own tight `lead` rate-limit bucket (`core/middleware.py`/`core/config.py`'s `rate_limit_lead_per_minute`, default 3/min/IP) since it's the one fully public endpoint in this API.
services/
