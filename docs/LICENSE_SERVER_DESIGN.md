...
2. **Two flagged assumptions in §14 need a direct confirm, not a guess:** whether Tier 3's metered billing genuinely needs ad-hoc per-event charging (vs. Helcim's subscription/payment-plan-shaped Recurring API, which is a different primitive), and whether "no updates" for Tier 3 means no new/premium features (requiring new per-tier feature-gating — nothing like it exists in the codebase today) rather than something else.
...
The three tiers replace `core`/`coop` as the ongoing paid options; `trial` (§3) stays a separate, time-limited, full-featured evaluation a prospective customer moves through before picking one of the three — not a fourth ongoing tier.
