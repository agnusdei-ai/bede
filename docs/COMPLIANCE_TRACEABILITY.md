...
| R6 | A family can delete a child's biometric/config data | Deletion rights for minors' biometric data (general good practice, not a specific statute) | `DELETE /voice/profiles/{student_name}` (`routers/voice.py`), `DELETE /pod/configs/{student_name}` (`routers/pod.py`) — both `require_parent` | `tests/test_voice_router.py` (voice side) + `tests/test_pod_router.py::test_delete_removes_the_row` / `::test_delete_only_removes_the_named_student` / `::test_delete_is_a_no_op_for_an_unknown_student` (pod side) — both endpoints now covered | none dedicated | 🟡 Both halves tested; pending merge into `main`. Still only per-item deletion exists — no single "erase this family's account entirely" action (checklist §2) |
...
- **R1/R2/R3/R6/R9/R10/R11's branch-only status is the same finding as checklist §0**, restated here as a compliance fact rather than a git-hygiene one: none of this session's consent-gate, sanitizer-integration, audit-log, deletion, or upload-limit tests are live in `main`'s CI until the branch merges. An attorney or auditor reviewing "what's actually deployed and tested" today would see none of them.
