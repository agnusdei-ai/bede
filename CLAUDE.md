...
**Reading presentation and break rhythm — the half `learning_support` structurally could not reach (`homeschool-tutor/src/utils/readingPresentation.ts`, `SessionConfig.letter_spacing`/`line_spacing`/`text_size`/`frequent_break_offers`):** `learning_support` goes to the PROMPT and nothing else, so it changes what Bede *says* and cannot change what the screen *looks like*. A parent who typed "bigger text with more space between the letters" got nothing — Bede might mention it, and the app rendered identically. For a child whose obstacle is reading the screen rather than understanding the lesson, that is most of the accommodation missing. Four fields close it, all defaulting to today's rendering byte for byte, all parent-set and per-student (unlike `useChatTheme`'s background/bubble colours, which are the CHILD's own per-device choice — an accommodation that only worked on one tablet would fail silently the moment someone changed rooms). `readingStyle()` returns an inline style for `TutorSession.tsx`'s `<main>`, which already wraps both the chat and the break cards — but **inheritance alone was not enough, and the first cut shipped two of the four settings inert.** `letter-spacing`/`word-spacing` inherit cleanly (nothing downstream sets them). `font-size`/`line-height` do not: every element that renders lesson text carried `text-sm leading-relaxed`, which sets both ON THE ELEMENT, and a property set on an element always beats one inherited from an ancestor however specific the ancestor's rule. Measured in real Chromium: `<main>` at 21.6px/45.36px while the chat bubble rendering every word of the lesson sat at 16.1px/26.16px — the two weaker settings reached the surrounding chrome and nothing a child reads. So those two ALSO travel as custom properties (`--bede-text-size`/`--bede-line-height`), which the text's own classes consume (`text-[length:var(--bede-text-size,1.00625rem)] leading-[var(--bede-line-height,1.625)]`) on all five text-bearing elements — messages, tool cards, the live transcript, the transcribing status, and the voice-review step. The fallbacks are exactly what those classes used to compile to *in this project's own overridden scale* (`text-sm` is 1.00625rem here, not stock Tailwind's 0.875rem), so a family with no setting renders byte-identically — verified in Chromium, not argued. `DEFAULT_TEXT_SIZE`/`DEFAULT_LINE_HEIGHT` are what the `LINE`/`SIZE` maps' own `normal` entries reference rather than restate, so the base and the fallback are one fact and cannot drift; a test reads `tailwind.config.js` and fails if the literal in the class name stops matching `text-sm`. **jsdom evaluates no cascade**, so the shipped defect was structurally invisible to every component test — the guards that exist now assert the contract that makes the cascade work (the classes are present, nothing shadows them, the fallbacks match), and the rendering itself is checked by hand in real Chromium, the same posture `HandwritingCanvas.tsx`'s `short:sr-only` fix carries. **Deliberately NOT a "dyslexia mode"**, and that is decision-register entry 24 with the evidence in `docs/ACCESSIBILITY_RESEARCH.md`: naming a setting after a condition would make this software hold a diagnosis (`_learning_support_note` already forbids Bede from doing so in words), it would fit two children with the same diagnosis equally badly, and the published objection to the strongest result here (Skottun and Skoyles 2012) argues the spacing benefit may belong to poor readers *generally* — an argument FOR a plain setting rather than one behind a diagnosis a family must first obtain.

**The four are not equally well founded, and each is offered to a parent in the words its evidence actually supports.** `letter_spacing` is strong (Zorzi et al. 2012, PNAS: doubled accuracy, >20% faster, replicated across Italian and French) and is the one described as an accommodation; it **carries word spacing with it as one setting**, because widening letters without widening words erodes the boundary that marks where a word ends, and two separate knobs would let a parent reach that bad combination by accident. `line_spacing` is design guidance only ("helps some readers keep their place," never "research shows"). `text_size` is offered as a **preference and explicitly not an accommodation** — the direction reverses with age (Katzir et al. 2013: smaller type RAISED comprehension for fifth graders while lowering it for second graders), so a parent may want it and Bede does not get to claim it helps. **There is deliberately no dyslexia font**: peer-reviewed studies of OpenDyslexic find no improvement in rate or accuracy, sometimes worse than Arial, and the IDA's position is that no reliable evidence supports them — `readingPresentation.test.ts` fails if a `fontFamily` is ever added, so the most-requested feature in this space cannot be shipped on request without someone reading why it was refused.
...
**None of the four reaches the prompt, and that is asserted rather than intended.** `test_the_presentation_settings_never_reach_the_prompt` scans the whole of `services/ai_service.py` for their names, because a model told "this student reads with wider letter spacing" has been handed exactly the fact `_learning_support_note`'s "the child is never told" rule exists to keep out of the conversation — and the friendly thing to do with that fact, mentioning it kindly, is the failure. `test_no_setting_names_a_condition` scans **every** field on `SessionConfig` (not a hardcoded list, which someone satisfies by not adding to it) for diagnosis words. Parent-facing docs: `docs/PARENT_SETUP.md` §5 for where the settings live, and `docs/SPECIAL_NEEDS.md` for the fuller walkthrough — which settings to reach for first, a "my child…" index, the five narration routes, and what Bede honestly cannot do. `docs/CHILD_GUIDE.md` is deliberately untouched: the child is never told any of this is on.
