...
// Was 4000ms. Lowered after a real debug-panel trace (demo app, same hook,
// mirrored here): a child's genuine, several-seconds-long answer to Bede
// produced ZERO native results (no interim, no final — the exact "Safari
// can accept the mic press and never fire ONE SINGLE onresult" failure mode
// described below) on two separate presses in the same session, both
// released at ~3.3-3.5s — just under the old 4000ms threshold, so the
// watchdog never got a chance to switch to the Whisper fallback before the
// child let go, and the whole answer was lost with nothing sent to Bede at
// all. Safe to lower: this watchdog is PERMANENTLY disarmed the moment even
// one interim result ever arrives (see the interim effect below), so
// shortening it only affects the case where native has produced literally
// nothing yet — never a hold that's actually in progress and making progress.
...
export function useHybridVoiceInput({ token, onFinal, language = 'en-US' }: Options) {
  ...
  // report back) with nothing telling the child or parent why the mic
  // button just stopped doing anything. Callers (SocraticChat/App) show
  ...
  const release = useCallback(() => {
    ...
    if (modeRef.current === 'native') {
      ...
      if (text) {
        ...
        // onNoSpeech above). Confirmed via a real debug-panel trace (demo
        // app, same hook, mirrored here): this used to just silently send
        // nothing, leaving the child's whole answer lost with no sign
        // anything went wrong.
