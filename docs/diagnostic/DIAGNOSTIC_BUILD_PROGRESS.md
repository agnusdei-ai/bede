...
| 3 — Loop integration | in progress (1/3 units) | — |
| 4 — Parent surface | not started | — |
| 5 — Validation & tuning | not started | — |

**Current status:** Unit 3.1 (`record_skill_evidence` tool + handler + dispatcher branch) merged. Next: unit 3.2 (`<diagnostic_guidance>` prompt injection).
...
| 2026-07-12 | 3.1 | This unit does NOT add the `<diagnostic_guidance>` static-prompt block or any subject-context mention of `record_skill_evidence` — Bede has no prompt-level awareness the tool exists yet. | Strict unit scoping per the progress table: 3.2 owns the prompt injection. Mirrors how 2.1 shipped ORM-only and 2.2 shipped persistence-only — each unit is a real, independently-tested slice; the tool is inert (never called by the model) until 3.2 lands, which is fine since nothing depends on it being invoked yet. |
...
Check output (`pytest tests/test_record_skill_evidence.py -v`): 7/7 passed (6 original + 1 added during the review pass). Full backend suite: 277 passed, 7 skipped, 1 pre-existing unrelated failure (`numpy` missing) — critically, the pre-existing `tests/test_ai_service_streaming.py` suite (which exercises the full streaming/tool-dispatch machinery for every other tool) passed unchanged, confirming the child's SSE stream is untouched for every tool this unit didn't add.

An 8-angle code review ran before merge; 4 of 8 finder agents hit a session-wide API rate limit mid-run and failed (not a code issue) — completed the remaining angles directly rather than retrying against a live limit. Found and fixed one real test-coverage gap (a hallucinated-but-well-formed `probe_id`, more realistic than the malformed-outcome case already covered, confirmed end-to-end to be a safe no-op) and confirmed as a positive finding that `_record_skill_evidence`'s try/except is the sole exception safety net in the whole dispatch chain. See Decisions Log 2026-07-12 (unit 3.1 review) for the full account and the 2 minor items deferred.

Before scoping this unit, re-verified `ai_service.py`'s actual current structure against the design doc's §7 sketch (see Decisions Log) since Appendix A/§5's line-number citations were already flagged stale — confirmed the substance (dispatcher shape, demo `db=None`, tools-block caching) still holds; only line numbers had drifted.

Deliverable: `record_skill_evidence` tool definition appended to `TUTOR_TOOLS` (matching design doc §7.1's schema exactly); `_record_skill_evidence(db, config, subject, tool_input)` helper mirroring `_save_assessment`'s demo-role guard and try/except-log-swallow contract, but returning nothing at all (stricter than `assess_narration`'s minimal child-ignored SSE event); one new branch in `_dispatch_completed_tool_call` returning `(None, None)` unconditionally. No `<diagnostic_guidance>` prompt injection yet — the tool is inert (never invoked by the model) until unit 3.2 tells Bede it exists; see Decisions Log for why that's fine at this stage.

Verified anchors: `_dispatch_completed_tool_call` returns exactly `(None, None)` for `record_skill_evidence` under every condition tested (valid input, `db=None`, malformed input) — proving the child's SSE stream is genuinely never touched, not just usually; `db=None` (the demo role's actual value, verified directly against `routers/tutor.py`'s `chat()`) is confirmed to never even reach `process_evidence` (mocked and asserted not-called, not just "returns early" by inspection); a non-mathematics subject is confirmed the same way; a genuinely malformed outcome value is confirmed to write zero rows rather than just "not raise"; a valid math-subject call is confirmed to persist a real `MasteryProfile` row end-to-end against the same real async SQLite engine unit 2.2's tests use.
