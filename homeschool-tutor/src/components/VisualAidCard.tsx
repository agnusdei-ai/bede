 ...
 * retrying once on a transient failure (see fetchWikiSummary above). That
 * keeps the curated catalog resilient to file-path changes on the image
 * host, at the cost of a runtime dependency on Wikipedia's API from the
 * child's device. If both attempts fail, or the lookup succeeds with no
 * image, this degrades to a plain captioned card — never a broken-image icon.
 */
