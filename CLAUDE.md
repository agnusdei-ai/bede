...
core/
  ...
  database.py        Async SQLAlchemy engine, ORM models (EncryptionConfig, VoiceProfile, StudentConfig, AuditLog, LearnerProfile, LearnerBehaviorCheck, MasteryProfile, ParentSecurityKey, ParentTotpConfig, ParentCredentialOverride, ParentRecoveryCode, ParentLoginLockout, and more — non-exhaustive list)
...
routers/
  ...
  mfa.py             Parent MFA enrollment/login-completion (FIDO2 security keys + TOTP, services/mfa_service.py) plus, as of "Parent account lockout & recovery" below: POST /mfa/change-password (a full parent session changing its own password — core/parent_credential.py), POST /mfa/recovery-code/enroll + DELETE /mfa/recovery-code (the "PIN" leg of account recovery, services/parent_recovery.py). GET /mfa/status now also reports recovery_code_enabled.
...
services/
  ...
  parent_recovery.py  The recovery code — a high-entropy, randomly-generated backup credential (shown once at enrollment, hashed via core/credential_hash.py) — one of the >=2 factors routers/recovery.py's account-recovery flow requires. Deliberately independent of both PARENT_PASSWORD and CHILD_PIN. See "Parent account lockout & recovery" below.
...
- **Account recovery** (`services/parent_recovery.py`, `routers/recovery.py`) — a locked-out or password-forgetting parent's way back in without server access. Requires proving **at least 2** of three independent factors, never just one: a recovery code (a high-entropy backup credential, shown once at enrollment via `POST /mfa/recovery-code/enroll`, hashed like the password override — deliberately independent of both `PARENT_PASSWORD` and `CHILD_PIN` so a leak of one doesn't expose the others), TOTP, or WebAuthn (both reusing `services/mfa_service.py` unchanged — a recovery-flow WebAuthn ceremony is identical to a login one). `GET /auth/recovery/methods` is public and reports only booleans (which factors are enrolled), never enough to help guess anything. Proving 2 factors issues a narrowly-scoped `parent_recovery` token (`core/deps.py`'s `require_parent_recovery`) good for exactly one call: `POST /auth/recovery/reset-password`.
...
components/
  ...
  ParentSecuritySettings.tsx  Collapsible parent-settings panel (rendered from `ParentSetup.tsx`): security keys, TOTP, the account-recovery code (see "Parent account lockout & recovery" above — needs 2 of 3 factors enrolled to ever be usable), and change-password. Change-password calls `logout()` + navigates to `/` immediately on success rather than waiting for the next request's inevitable 401 — the credentials_version bump invalidates the very session making the change too.
  AccountRecovery.tsx  Login.tsx's "Forgot password?" destination — public, no session required. Fetches which recovery factors are enrolled (`GET /auth/recovery/methods`), collects >=2, verifies, then sets a new password. Reports plainly when recovery isn't set up (fewer than 2 factors enrolled) rather than pretending it might work. Not currently localized (plain English strings), same disclosed gap as `FeedbackModal.tsx` above — a rare, emergency-only flow, not blocking on a translation pass.
