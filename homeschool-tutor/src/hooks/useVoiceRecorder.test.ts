...
describe('useVoiceRecorder getUserMedia failure reporting', () => {
  ...
  // startRecording just returned early (`if (!stream) return`), leaving
  // whatever mode useHybridVoiceInput had already set (e.g. 'recording')
  // stuck forever with no event ever telling it to go back to idle.
