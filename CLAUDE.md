...
- **Mistral** (`mistral_model`, default `mistral-large-latest`) and **OpenAI**
  (`openai_model`, default `gpt-4.1-mini`) — the models the public Render demo
  actually uses in production, since its `BEDE_ADAPTER_ORDER=mistral,openai`
  (Mistral primary, OpenAI as the boot-time fallback if `MISTRAL_API_KEY` is
  ever unset).
