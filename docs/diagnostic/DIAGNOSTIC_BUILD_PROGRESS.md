...
Check output (`pytest tests/diagnostic/test_facade_persisted.py -v`): 8/8 passed — real round trip against a genuine async SQLite engine (`aiosqlite`), with `core.encryption.initialize_encryption()` run for real so every encrypt/decrypt is genuine AES-256-GCM. Full `pytest tests/diagnostic/ -v`: 124/124 passed (116 prior + 8 new), no regressions. Full backend suite: 258 passed, 7 skipped, 1 pre-existing unrelated failure (`numpy` missing) — none introduced by this change.
