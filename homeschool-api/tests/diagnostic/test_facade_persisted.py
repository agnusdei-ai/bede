...
async def test_persisted_vector_matches_the_in_memory_computation_exactly(db_session):
    ...
    assert decrypt_json(
        row.profile_enc,
        student_aad("mastery_profiles", "profile_enc", row.student_name, row.subject_area),
...
async def test_corrupted_existing_row_degrades_to_cold_start_instead_of_raising(db_session):
    ...
    assert decrypt_json(
        rows[0].profile_enc,
        student_aad("mastery_profiles", "profile_enc", rows[0].student_name, rows[0].subject_area),
...
async def test_evidence_log_gets_exactly_one_row_per_call_when_flag_is_on(db_session, monkeypatch):
    ...
    deltas = decrypt_json(
        rows[0].delta_enc,
        student_aad("diagnostic_evidence_log", "delta_enc", rows[0].student_name),
        await student_keys.get_existing(db_session, rows[0].student_name),
    )
...
async def test_evidence_log_row_never_contains_the_raw_outcome_or_confidence(db_session, monkeypatch):
    ...
    deltas = decrypt_json(
        row.delta_enc,
        student_aad("diagnostic_evidence_log", "delta_enc", row.student_name),
        await student_keys.get_existing(db_session, row.student_name),
    )
    for delta in deltas:
