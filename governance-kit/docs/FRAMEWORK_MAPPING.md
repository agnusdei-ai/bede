...
| **LLM10** | Unbounded Consumption | [G06](../prompts/G06-tool-use-discipline.md) loop caps; per-actor message quotas | Strong once both are present. A **rate limit bounds request rate, never aggregate spend**: a session sustained at your rate ceiling for a full token lifetime is unbounded cost. You need a hard per-actor ceiling as well, checked *before* the model call so an over-quota turn costs a database read rather than an inference. |
