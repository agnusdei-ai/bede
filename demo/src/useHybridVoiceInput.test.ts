...
describe('useHybridVoiceInput exposes prewarm/cancelPrewarm (demo)', () => {
  // The hold-to-talk mic effect (App.tsx) needs to open the microphone
  // proactively, before any press — see useVoiceRecorder.ts's own prewarm()
  // comment for why a call issued synchronously at press-time can't close
  // that gap by itself. This hook must pass the recorder's own prewarm/
  // cancelPrewarm straight through rather than only using them internally.
