...
failure is usually not a greeting, it is an affirmation. Name affirmation
specifically.

**Consider whether you need this at all.** If every tool in your loop is
non-reactable — the loop always exits after one round — this rule costs prompt
tokens for a case that cannot occur. Add it when you add your first reactable
tool, not before.
