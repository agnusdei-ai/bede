 ...
 * (demo/, since removed) — kept around this time since "I can't see what
 * the voice UI is actually doing" is a recurring class of report, not a
 * one-off. logDebug() is a no-op with effectively zero cost when the
 * panel is closed (DebugOverlay isn't subscribed, so entries accumulate
 * in the ring buffer and are simply never read).
 */
