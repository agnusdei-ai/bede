...
describe('demo/app parity', () => {
  it('uses the same numbers as the app, because the demo sells the app', async () => {
    // Not a style preference. A visitor judging Bede by the demo is judging
    // the product, so a demo that endpoints on a different silence window
    // than the app is quietly demoing something the family will not get.
    const app = await import('../../homeschool-tutor/src/utils/endpointing')
    const mine = await import('./endpointing')
    for (const k of [
      'SILENCE_LEVEL', 'MIN_SPEECH_MS', 'TRAILING_SILENCE_MS',
      'NO_SPEECH_TIMEOUT_MS', 'SAMPLE_INTERVAL_MS',
    ] as const) {
      expect(mine[k], k).toBe(app[k])
    }
  })
})
