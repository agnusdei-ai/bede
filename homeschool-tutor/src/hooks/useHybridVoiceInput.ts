// Mirror of demo/src/useHybridVoiceInput.ts for the homeschool-tutor app.
...
export function useHybridVoiceInput({ token, onFinal, language = 'en-US' }: Options) {
  ...
  // child or parent why the mic button just stopped doing anything. Callers
  // show this once, then call clearMicError.
  ...
  // Reacts to `mode` (not individual start/stop call sites) so every path
  // that ends listening is covered by one effect instead of needing a call
  // threaded into each of them individually.
  useEffect(() => {
    if (mode === 'recording') {
  ...
  const _start = useCallback(() => {
    ...
    // late for iOS Safari's own getUserMedia timing. See utils/audioSession.ts
    // and docs/VOICE_SETUP.md's audio-session-race troubleshooting sections.
    enterRecordingAudioSession()
    setMode('recording')
