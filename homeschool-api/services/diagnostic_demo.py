...
the demo's in-memory, single-session code store (core/demo_code_session.py).

Deliberately NOT the production integration described in
docs/diagnostic/DIAGNOSTIC_ENGINE_DESIGN.md §7-§9 (that path is
db-backed via services.diagnostic.process_evidence, persists across
sessions, and isn't built yet). This module exists because the demo's
scope for this feature is a single-session preview only: evidence
recorded during one demo code's session builds a MasteryVector that lives
exactly as long as that code does (core/demo_code_session.py's normal TTL/
eviction rules) and is gone forever the moment it expires or the visitor
logs out — never written to a database, never seen across two different
codes. Production (homeschool-tutor, the parent/child roles) is untouched
...
async def record_skill_evidence_demo(
    ...
    """In-memory equivalent of services.diagnostic.process_evidence, for the
    demo role only — cold-starts a vector for this code if it doesn't have
    one yet, applies one observation, and stores the result back onto the
...
def get_mastery_summary_demo(code: str, student_name: str, subject_area: str = "mathematics") -> dict | None:
    ...
    evidence_count = get_mastery_evidence_count(code)
    return {
