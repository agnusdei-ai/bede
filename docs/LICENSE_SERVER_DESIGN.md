...
4. **Tier 0 issuance mechanism:** a real License-Server-issued `LICENSE_KEY` (consistent, but arguably heavier than a free account needs) or a lighter `homeschool-api`-internal mechanism closer to `demo_code` but persistent (§14).
5. **Tier 0 summary-diagnostic cadence:** one-time-ever, or unlimited-but-always-summary-only (§14) — a materially different product, not just wording.
...
The three paid tiers replace `core`/`coop`; `trial` (§3) stays a separate, time-limited, **full-featured** evaluation a prospective customer moves through before picking one of the three. **Tier 0** below is a *third* kind of access, different from both: free and perpetual (not time-limited like `trial`), but deliberately **not** full-featured (unlike `trial`) — a permanent, low-commitment top of the funnel rather than a countdown toward a purchase decision.

- **Tier 0 — Community (free, perpetual, no card).** Full access to the same weekly community check-ins Tier 2 offers — cheap to deliver at any scale, builds affinity before a purchase decision. Diagnostics are capped at a single point-in-time **summary snapshot**: no per-skill breakdown, no trend over time, no access to `services/diagnostic/`'s real continuous mastery-tracking engine. This is the literal "flying blind" a free user should experience by design — one surface-level read, no ongoing signal — with the upgrade prompt sitting right where that gap is felt (next to the summary: "see how this changes over time, by skill"). Two open forks, not assumed:
  - **Issuance mechanism is unresolved.** A real License-Server-issued signed `LICENSE_KEY` (`licenses.tier = "tier0"`, consistent with §12's "every license through one system" decision — and it makes Tier 0→paid conversion just a normal upgrade, no account migration), or a lighter mechanism entirely inside `homeschool-api` closer to today's `demo_code` concept but persistent, since there's no payment or revocation relationship to actually manage for a free account. Leaning toward the former for consistency; flagged, not decided.
  - **"Summary snapshot" cadence is unresolved.** A one-time-ever diagnostic, or an unlimited-but-always-summary-only diagnostic a free user can re-run indefinitely? Materially different product, not just a wording difference.
...
- **Tier 3 — Metered, access-only.** Platform access with **no** live training, **no** community check-ins, and (pending confirmation, §13) **no** access to new/premium features added after signup — a deliberately bare offering. Billed **per diagnostic test, $15/test**, not a flat subscription. This is the one tier that changes §6–§7's design, not just the tier label:
  ...
  - **"No paid learning outcomes"** is read here as: Tier 3 carries no outcome-based guarantee or premium reporting layer beyond the raw diagnostic result itself — consistent with "access-only," not a new mechanism to design.

### 14.1 Feature-gating is now real, cross-cutting scope — not one tier's edge case

Three tiers now depend on differentiated feature access, not just Tier 3's originally-flagged question: Tier 0 gets summary-only diagnostics, Tier 3 gets no new/premium features (pending §13's confirmation), Tiers 1/2 get full capabilities. **Nothing like this exists in the codebase today** — `core/licensing.py`'s `tier` field currently gates exactly two things (the license-required boot gate, and `routers/pod.py`'s `seats` cap), never *which features run*. This needs its own small piece of design before Phase 2 client-integration work starts, not an implicit assumption:
...
- At minimum, one gate point already identified: wherever `services/diagnostic/`'s full mastery-profile view is rendered (`Progress.tsx`, per `CLAUDE.md`'s architecture notes) needs a tier check — summary-only for Tier 0, full for everyone else. Tier 3's "no new features" gate is a broader, less-defined surface pending §13's confirmation of what "new features" actually excludes.
