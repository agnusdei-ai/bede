...
async def _sync_behavior_check(db: AsyncSession, student_name: str, old_style: str | None, new_style: str) -> None:
    ...
    if new_style in TRACKABLE_STYLES:
        ...
        count_enc = encrypt_json({"count": 0}, _behavior_aad(student_name))
        if row is None:
...
async def get_assessments(
    ...
    return [decrypt_json(row.assessment_enc, _assessment_aad(row.student_name)) for row in rows]
...
async def get_profile(
    ...
    return decrypt_json(row.profile_enc, _profile_aad(student_name))
...
async def build_profile(
    ...
    assessments = [decrypt_json(row.assessment_enc, _assessment_aad(row.student_name)) for row in rows]
    ...
    row = existing.scalar_one_or_none()
    old_processing_style = decrypt_json(row.profile_enc, _profile_aad(student_name)).get("processing_style") if row is not None else None
    if row is None:
...
async def get_behavior_check(
    ...
    if row is None:
        return None
    return {
        "count": decrypt_json(row.count_enc, _behavior_aad(student_name))["count"],
